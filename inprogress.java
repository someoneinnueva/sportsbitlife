...
class JuniorRankingStrategy implements RankingStrategy { // OOP: Strategy pattern
  int calculateRanking(int points, int age) {
    if (points >= 2000) return (int)random(1, 5);
    if (points >= 1000) return (int)random(5, 25);
    if (points >= 500)  return (int)random(25, 80);
    if (points >= 200)  return (int)random(80, 200);
...
abstract class BaseScreen { // OOP: Inheritance
  ...
  void drawEventPanel(GameEvent evt, float x, float y, float w) {
    ...
    textSize(17);
    textAlign(LEFT, TOP);
    drawWrappedText(evt.headline == null ? "" : evt.headline, x + 14, y + 40, w - 28, 22);
    ...
    if (evt.choices != null) {
      ...
      for (int i = 0; i < min(evt.choices.size(), 3); i++) {
        ...
        boolean     hover = theme.isHover(hoverX, hoverY, x + 10, cy, w - 20, 60);
        ...
        rect(x + 10, cy, w - 20, 60, 8);
        noStroke();

        // Shine overlay when hovered
        if (hover) {
          fill(255, 255, 255, 20);
          rect(x + 12, cy + 2, w - 24, 18, 6);
        }

        // Number circle
        fill(hover ? color(10, 12, 18) : ba, hover ? 255 : 180);
        noStroke();
        ellipse(x + 30, cy + 30, 24, 24);
        fill(hover ? color(10, 12, 18) : color(10, 12, 18));
        textSize(11);
        ...
        textSize(13);
        textAlign(LEFT, TOP);
        text(ch.label == null ? "" : ch.label, x + 48, cy + 12);
  ...
  private String formatMoneyAbs(float m) {
...
class AIScenarioEngine {
  ...
  String buildWeeklyPrompt(String playerCtx, Sport sport) {
    ...
    return "You are the event engine for a " + cfg.name + " career simulator styled like BitLife.\n\n" +
      "Player: " + playerCtx + "\n\n" +
      "Generate ONE life event. Mix career events 40% with personal life events 60%.\n" +
      "Categories (pick based on player context): career milestone, training decision, " +
      "romance/relationship, family crisis, media scandal, legal trouble, financial decision, " +
      "health scare, social media drama, rival conflict, endorsement deal, " +
      "personal growth moment, friendship event, housing/lifestyle choice.\n\n" +
      "JSON ONLY (no markdown):\n" +
      "{\"type\":\"CAREER_CHOICE\",\"headline\":\"max 8 words\",\"description\":\"2-3 sentences\"," +
      "\"choices\":[{\"label\":\"choice\",\"description\":\"outcome\"," +
      "\"effects\":{\"confidence\":0,\"fatigue\":0,\"mental\":0,\"reputation\":0," +
      "\"rankingPoints\":0,\"money\":0,\"family\":0,\"happiness\":0,\"relationship\":0}}]}\n\n" +
      "Exactly 3 choices. Ranges: confidence/mental/fatigue/family/happiness: -20 to +20; " +
      "rankingPoints: -50 to +50; money: -200 to +1000 ($K). Be sport-specific and dramatic.";
  }
  ...
  // IMPROVEMENT 1 — Expanded fallback event pool (15 events)
  // OOP: Factory pattern — GameEventFactory delegates to typed builder methods.
  // When Claude API is unavailable, pick randomly from a pool of 15 varied life events.
  GameEvent fallbackEvent() {
    int pick = (int)random(15);
    switch (pick) {
      // Original 7 events — via GameEventFactory builder methods
      ...
      case 6:  return buildFallback_Burnout();
      ...
      default: return buildFallback_ControversialEndorsement();
    }
  }
...
class GameEngine { // OOP: Composition
  ...
  void updateRanking() {
    ...
    if (age < 18) {
      ...
      p.career.juniorRanking = (int)lerp(p.career.juniorRanking, strategy.calculateRanking(p.career.rankingPoints, age), 0.3);
      ...
      p.career.worldRanking = (int)lerp(p.career.worldRanking, strategy.calculateRanking(p.career.rankingPoints, age), 0.4);
    }
  }
...
class BasketballCalendarBuilder implements SportCalendarBuilder {
  void buildSchedule(ArrayList<Tournament> schedule) {
    String[] teams={"Lakers","Warriors","Celtics","Heat","Bucks","Suns","Nuggets","Clippers","Spurs","76ers"};
...
class SoccerCalendarBuilder implements SportCalendarBuilder {
  void buildSchedule(ArrayList<Tournament> schedule) {
    String[] teams={"Man City","Arsenal","Chelsea","Liverpool","Tottenham","Man Utd","Newcastle","Aston Villa","Brighton","Wolves"};
    for (int w=2;w<=39;w++) schedule.add(new Tournament("vs. "+teams[(int)random(teams.length)],TournamentTier.ATP_250,Surface.HARD,2,100000,w,Sport.SOCCER));
    schedule.add(new Tournament("Cup Final",TournamentTier.GRAND_SLAM,Surface.HARD,2,1000000,50,Sport.SOCCER));
  }
}
